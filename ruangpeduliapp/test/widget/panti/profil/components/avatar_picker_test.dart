...
// ─── AvatarPicker (public widget, testable) ───────────────────────────────────

class AvatarPicker extends StatelessWidget {
  /// URL gambar dari network. Boleh null / kosong jika belum ada foto.
  final String? imageUrl;

  /// File lokal yang dipilih dari gallery (opsional, override imageUrl).
  final File? localImage;

  /// Apakah foto sedang dihapus (tampilkan placeholder).
  final bool removed;

  /// Callback saat widget di-tap.
  ...
  Widget build(BuildContext context) {
    final bool hasPhoto =
        localImage != null || (imageUrl != null && imageUrl!.isNotEmpty && !removed);

    DecorationImage? decorationImage;
    if (localImage != null) {
      decorationImage = DecorationImage(image: FileImage(localImage!), fit: BoxFit.cover);
    } else if (imageUrl != null && imageUrl!.isNotEmpty && !removed) {
      decorationImage = DecorationImage(image: NetworkImage(imageUrl!), fit: BoxFit.cover);
    }
...
class _EditProfilePantiState extends State<EditProfilePanti> {
  ...
  void _showPhotoOptions() {
    ...
    final hasPhoto =
        _selectedImage != null || (currentPicUrl != null && !_removePhoto);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _buildTextField(
                controller: _usernameController, hint: '@usernamepanti'),
            const SizedBox(height: 18),
...
class _ChangeEmailDialog extends StatefulWidget {
  ...
  const _ChangeEmailDialog(
      {required this.userId, required this.currentEmail});
...
class _ChangeEmailDialogState extends State<_ChangeEmailDialog> {
  ...
  Future<void> _sendNewEmailOtp() async {
    ...
    if (otp.length != 5) {
      setState(() => _error = 'Masukkan 5 digit kode OTP');
      return;
    }
  ...
  Future<void> _confirmChange() async {
    ...
    try {
      final confirmed =
          await _api.confirmEmailChange(widget.userId, otpNew, newEmail);
      if (!mounted) return;
  ...
  Widget _buildStepIndicator() {
    ...
    final currentIndex =
        _step.index < 2 ? _step.index ~/ 1 : _step.index - 1;
    return Row(
                ...
                child: Column(
                  children: [
                    CircleAvatar(
                      radius: 10,
                      backgroundColor:
                          active ? kPink : const Color(0xFFDDDDDD),
                      child: Text('${i + 1}',
                          style: TextStyle(
                              fontSize: 10,
                              color:
                    ...
                    Text(steps[i],
                        ...
                        style: TextStyle(
                            fontSize: 9,
                            color: active ? kPink : Colors.grey)),
                  ],
                ),
              ),
              if (i < steps.length - 1)
    ...
    switch (_step) {
      case _EmailStep.sendCurrentOtp:
        return Column(
            ...
            children: [
              const Text(
                  'Kode verifikasi akan dikirim ke email kamu saat ini:',
                  style: TextStyle(
            ...
            children: [
              ...
              TextButton(
                ...
                child: const Text('Kirim ulang OTP',
                    style: TextStyle(fontSize: 13, color: kPink)),
              ),
            ]);
